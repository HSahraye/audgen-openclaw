...
**Future secondary ICPs (Phase 7+):**
- MSPs (managed-service providers) selling IT services to SMBs.
- B2B consultants packaging recurring advisory contracts.
- Recruiters / staffing agencies selling outbound to hiring managers.
- Insurance brokers running outbound to local commercial accounts.
- Home-service sales teams (HVAC, plumbing, roofing) running outbound
  to commercial property managers.
- Franchise sales teams qualifying franchise applicants.
- SaaS SDR teams selling SMB-priced products into the same buyer profile.
...
Every workspace dashboard must surface these metrics. Phase 1 ships the
raw counters; Phase 2 adds the rates and conversions; Phase 5 adds the
breakdowns.

**Raw counters (Phase 1):**
- leads imported
- leads scored
- audits generated
- outreach prepared
...
**Rates (Phase 2):**
- connection rate = contacted / imported
- reply rate = replied / contacted
- booked-call rate = booked / replied
- proposal rate = proposal / booked
- close rate = won / proposal
- revenue won (sum of `Opportunity.amount` where stage=won)
- pipeline value (sum of `Opportunity.amount` where stage in proposal/qualified)

**Breakdowns (Phase 5):**
- conversion by lead score band (0–20 / 21–40 / 41–60 / 61–80 / 81–100)
- conversion by vertical (smoke-shop, HVAC, dental, etc.)
- conversion by city / metro
- conversion by outreach template
- conversion by primary audit issue (mobile / CTA / reviews / etc.)
- per-rep activity and conversion (Phase 5b: team plans)

All metrics are workspace-scoped server-side. The dashboard never trusts
a workspaceId from the client.
