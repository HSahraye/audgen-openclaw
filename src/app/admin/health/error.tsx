...
export default function AdminHealthError({
  ...
  useEffect(() => {
    logger.error("admin_health_error_boundary", {
      ...
      <div className="w-full max-w-lg rounded-3xl border border-amber-200 bg-white p-8 shadow-sm">
        <div className="flex items-center gap-3 text-amber-700">
          ...
          <p className="text-xs font-black uppercase tracking-[0.2em]">
            Health view didn&rsquo;t load
          </p>
        </div>
        <h1 className="mt-3 text-2xl font-black text-slate-950">
          Couldn&rsquo;t fetch system health.
        </h1>
        <p className="mt-3 text-sm leading-relaxed text-slate-600">
          The underlying health probe at <code className="text-xs font-mono">/api/health</code> is the
          source of truth for uptime monitors. This admin dashboard is a
          convenience view on top.
        </p>
