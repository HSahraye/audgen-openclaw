...
function normalizeGooglePlaceToLead(
  ...
  const category = query.category?.trim() || place.primaryTypeDisplayName?.text || "Local Services";
  ...
  async fetchLeads(query: LiveAdapterQuery = {}): Promise<LiveAdapterFetchResult> {
    ...
    if (isSandboxMode()) {
      return {
        ...
        blocked: false,
        message: "Sandbox mode enabled. Returning localized synthetic Google Places leads.",
      };
    }

    if (!isLiveConnectorEnabled()) {
      return {
        status: "SANDBOX_MODE",
        leads: generateSandboxLeads("google_places", query),
        blocked: false,
        message: "Live connectors disabled by kill-switch. Falling back to sandbox lead synthesis.",
      };
    }
    ...
    const placeNames = (searchPayload.places ?? [])
      .map((item) => item.name)
    ...
    for (const placeName of placeNames) {
      const detailsRes = await fetch(`https://places.googleapis.com/v1/${placeName}`, {
        ...
        headers: {
          "X-Goog-Api-Key": apiKey,
          "X-Goog-FieldMask":
            "id,displayName,formattedAddress,nationalPhoneNumber,websiteUri,rating,userRatingCount,googleMapsUri,primaryTypeDisplayName",
        },
      });
      if (!detailsRes.ok) continue;
      const detail = (await detailsRes.json()) as GooglePlaceDetails;
      details.push(detail);
      if (details.length >= liveLimit) break;
    }

    const mapped = await mapGooglePlacesToLeadOpportunities(details.slice(0, MAX_LIVE_SEARCH_RESULTS), query);
    return {
      status: "READY",
